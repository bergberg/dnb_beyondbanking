...
```{r}
(Lung_GeneExpression_transposed <- read_fst(files_fst$Lung_GeneExpression, from = 1,to = 10000) %>% as_data_frame %>% 
  gather(SampleID, value = gene_expression, starts_with("TCGA")) %>% select(Gene,gene_expression,SampleID) %>%
   group_by(Gene, SampleID) %>% summarize_all(mean) %>% 
  spread(Gene,gene_expression) )
```
